...
#SBATCH --output=/projects/ttan/UBC-TMS/analysis_LsgACC/PALM/PALM_conmat_MDD_TMS-rsfMRI_rs-fMRI0_diff_with_covariates_noMT_34_subs_%j.txt
#SBATCH --array=1
...
sublistids="${DIR}/sublist_MADRS_34_subs.txt"

#assign full path for filename files, which is a list of path to rs-fMRI output in surface space
filename="$(find $DIR/filelists/TMS-fMRI_rs-fMRI0/filelist34*.txt -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/PALM_conmat_MDD_TMS-fMRI_rs-fMRI0_diff_with_covariates_noMT_/;s/.txt//')"
outdir="${DIR}/Results/$truncname"
desmat="$DIR/con_design/design_mat_active_baseline_with_covariates_noMT_34_subs.csv"
